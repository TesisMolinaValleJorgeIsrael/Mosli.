// js/app.js — video inicial (una vez) + UI XRD con gráfica blanca y zoom
(() => {
  /********* Parte A: manejo del video inicial (una sola reproducción) *********/
  ...
  // Cuando el video termine, mostrar UI
  if (video) {
    video.addEventListener('ended', () => {
      showMainUI();
    });
    // si por alguna razón el video no termina y quieres ocultarlo tras N segundos
    // puedes descomentar lo siguiente para ocultar después de 8s:
  ...
  function createChart(x, y){
    ...
    try {
      if (window?.Chart?.register && window?.chartjsPluginZoom) {
        Chart.register(chartjsPluginZoom);
      } else if (window?.ChartZoom) {
        Chart.register(window.ChartZoom);
      }
    } catch(e){ console.warn('No se pudo registrar plugin zoom:', e); }
    ...
    chart = new Chart(ctx, {
      ...
      options: {
        ...
        scales: {
          x: {
            title: { display: true, text: '2θ (deg)', color:'#222' },
            ticks: { color:'#222' },
            grid: { display:true, color:'rgba(0,0,0,0.08)' }
          },
          y: {
            title: { display: true, text: 'Intensidad (u.a.)', color:'#222' },
            ticks: { color:'#222' },
            grid: { display:true, color:'rgba(0,0,0,0.08)' }
          }
        },
  ...
  function overlayPeaksAndFits(){
    ...
    if (baseline) {
      ...
      chart.data.datasets.push({ label:'Baseline', data: baselineY, borderDash:[6,4], pointRadius:0, borderWidth:1, borderColor:'#888' });
    }
    if (procY && procY.length) {
      chart.data.datasets.push({ label:'Procesado', data: procY, tension:0.1, borderWidth:1, pointRadius:0, borderColor:'#ffb74d' });
    }
    for (const pk of peaks) {
      chart.data.datasets.push({ label:`Pico @ ${rawX[pk.index].toFixed(3)}`, data:[{x:rawX[pk.index], y:rawY[pk.index]}], showLine:false, pointRadius:6, backgroundColor:'#ef5350' });
    }
    if (selectedPeak && selectedPeak.fit) {
      ...
      chart.data.datasets.push({ label:'Fit (gauss)', data: yfit, borderWidth:2, pointRadius:0, borderColor:'#66bb6a' });
    }
    chart.update('none');
  }
  ...
  resetBaselineBtn && resetBaselineBtn.addEventListener('click', ()=> {
    baseline = null; procY = rawY.slice(); createChart(rawX, procY); overlayPeaksAndFits(); log('Baseline reseteada.');
  });
  ...
  downloadPNG && downloadPNG.addEventListener('click', ()=> {
    if (!chart) return;
    const url = chart.toBase64Image(); const a=document.createElement('a'); a.href=url; a.download='xrd_plot.png'; a.click();
  });

  resetZoomBtn && resetZoomBtn.addEventListener('click', ()=> {
    if (chart && chart.resetZoom) { chart.resetZoom(); log('Zoom reseteado'); }
    else if (chart) {
      try { chart.resetZoom(); log('Zoom reseteado'); } catch(e){ log('No se pudo resetear zoom (plugin no registrado)'); }
    }
  });

  // inicializar chart vacio
  createChart([0,1],[0,0]);
